...
namespace ECS
{
    ...
    void ECS::RemoveEntity(std::size_t entityId)
    {
        // auto it = std::find(p_entities.begin(), p_entities.end(), entity.id);

        // if (it == p_entities.end())
            throw ECSError("Entity with ID = " + std::to_string(entityId) + " does not exist");
        // p_entities.erase(it);
    }

    bool ECS::HasEntity(Entity) const
    {
