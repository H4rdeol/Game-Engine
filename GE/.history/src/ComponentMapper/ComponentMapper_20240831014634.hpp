            ...
            void AddComponent(const EntityId entity, C &component, Args... args)
            {
                ...
                if (p_componentsIndexes.contains(entity)) {
                    p_componentsIndexes[entity]
                        [std::size_tComponentsType::Position] = (int)index;
                } else {
                    p_componentsIndexes[entity].reserve(
                        static_cast<std::size_t>(ComponentsType::NbComponents));
                    for (std::size_t i = 0; i < static_cast<std::size_t>(ComponentsType::NbComponents); i++)
                        p_componentsIndexes[entity].push_back(-1);
                    p_componentsIndexes[entity]
                        [static_cast<std::size_t>(ComponentsType::Position)] = (int)index;
                }
            }
